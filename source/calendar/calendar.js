...
function getIconColor(deadline, priority) {
    ...
    if (diffDays < 0) {
        ...
        return '#FF000F'; // Red if within a week
    } else if (priority === "medium") {
        return '#FFD600'; // Yellow if between one and two weeks
    } else {
        return '#0AB73B'; // Green if over two weeks in future
    }
}
...
function addNotesAndProjectsToCalendar() {
    ...
    for (const [key, value] of Object.entries(projectTable)) {
        ...
        if (dayElement) {
            ...
            icon.style.backgroundColor = getIconColor(value.deadline, value.priority);
